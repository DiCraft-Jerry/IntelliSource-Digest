...
/**
 * 调用 AI 模型分析网页信息
 * @param {{ title: string, description: string, links: Array }} pageInfo - 页面关键信息
 * @param {{ apiUrl: string, apiKey: string, model: string }} config - API 配置
 * @returns {Promise<string>} AI 分析结果文本
 */
export async function summarizePageInfo(pageInfo, config) {
  ...
  try {
    ...
    if (!content) {
      // 诊断具体是哪个层级缺失
      if (!data.choices) {
        throw new Error('端点返回的 JSON 不兼容 OpenAI 格式：缺少 choices 字段，请确认 API 地址是否正确');
      }
      if (!data.choices[0]?.message) {
        throw new Error('端点返回的 JSON 不兼容 OpenAI 格式：choices 中缺少 message 字段');
      }
      throw new Error('端点返回了 message 但 content 为空，请确认模型名称是否正确');
    }

    return content;
  } catch (error) {
    if (error.name === 'AbortError') {
      throw new Error('AI 请求超时（30秒），请检查网络连接或 API 地址是否可访问');
    }
    // 网络错误（DNS 解析失败、连接拒绝等）
